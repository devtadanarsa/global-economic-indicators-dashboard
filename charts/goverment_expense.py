...
st.subheader("🏛️ Pemerintah (% of GDP)")

st.markdown("**Government Expense (% of GDP)**")
chart_exp = (
    alt.Chart(filtered)
    .mark_line(point=True, strokeWidth=3)
    .encode(
        x=alt.X("year:O", title="Tahun", axis=axis_x),
        y=alt.Y("Government Expense (% of GDP):Q", title="% of GDP", axis=axis_y),
        color=alt.Color("country_name:N", title="Negara"),
        tooltip=["country_name", "year", "Government Expense (% of GDP)"]
    )
